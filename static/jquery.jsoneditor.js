    ...
    function construct(opt, json, root, path) {
        ...
        for (var key in json) {
            ...
            var item     = $('<div>',   { 'class': 'item', 'data-path': path }),
                property = $(opt.propertyElement || '<input>', { 'class': 'property', 'readonly': 'readonly' }),
                value    = $(opt.valueElement || '<textarea>', { 'class': 'value'    });
            ...
            //Can't edit arrays and objects
            if (val.search(/^[\{\[]/) >= 0) {
                value.val(val).attr({'readonly': 'readonly',
                                     'class': 'hidden'})
            }
            value.val(val).attr('rows', '1')
            value.val(val).html(val);
